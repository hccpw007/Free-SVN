    ...
    >
      ...
      <div class="h-8 px-4 flex items-center justify-between shrink-0 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
        ...
        <div class="flex items-center gap-2">
          <button class="text-xs text-slate-400 hover:text-slate-600 flex items-center gap-0.5 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded" :aria-label="t('common.search')" :title="t('common.search')" @click="toggleSearch"><Search class="w-3 h-3" /></button>
          <button v-if="diffFile" class="text-xs text-slate-400 hover:text-blue-600 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded" :aria-label="t('file.openWithEditor')" :title="t('file.openWithEditor')" @click="openInExternalTool(diffFile.path)">{{ t('file.openWithEditor') }}</button>
          <button v-if="diffFile" class="text-xs text-slate-400 hover:text-blue-600 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded" :aria-label="t('file.copyFullPath')" :title="t('file.copyFullPath')" @click="copyFilePath(diffFile.path)">{{ t('file.copyFullPath') }}</button>
          <button :aria-label="t('file.diffPrev')" class="text-xs text-blue-600 hover:underline flex items-center gap-0.5 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded" @click="navDiff('prev')"><ChevronUp class="w-3 h-3" />{{ t('file.diffPrev') }}</button>
          <button :aria-label="t('file.diffNext')" class="text-xs text-blue-600 hover:underline flex items-center gap-0.5 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded" @click="navDiff('next')">{{ t('file.diffNext') }}<ChevronDown class="w-3 h-3" /></button>
          <button :aria-label="t('common.close')" class="p-0.5 rounded text-slate-400 hover:text-slate-600 focus:ring-2 focus:ring-blue-400 focus:outline-none" :title="t('common.close')" @click="closeDiff"><X class="w-4 h-4" /></button>
        </div>
      </div>
      <!-- 搜索输入框 -->
