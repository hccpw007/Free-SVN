 ...
 *    让调用方自行决定如何处理国际化。
 *
 * 错误码映射表：使用 ErrorCode 枚举派生映射，避免手工字符串拼写错误。
 * 每条映射包含对应的 i18n key 和英文回退文本，当语言包中未定义该 key 时，
 * 调用方应使用英文回退文本作为兜底。
 */
 ...
 * 英文回退使用方法：
 * 
 * translateError 返回的是 ErrorCode 枚举值（同时也是 i18n key）。
 * 调用方在 store 或组件中使用时，应先尝试从语言包获取翻译，
 * 若 key 在语言包中未定义，则使用 ErrorFallbackMap 中的英文回退：
 * 
 * ```ts
 * import { translateError, ErrorFallbackMap, ErrorCode } from '@/services/svn'
 * import { useI18n } from 'vue-i18n'
 * 
 * const { t, te } = useI18n()
 * const errorKey = translateError(someErrorMsg)
 * // 方式 A：如果语言包中已定义该 key，使用 t()；否则用英文 fallback
 * const displayMsg = te(errorKey) ? t(errorKey) : ErrorFallbackMap[errorKey as ErrorCode] ?? errorKey
 * ```
 * 
 * 该机制确保即使语言包更新滞后，用户也不会看到原始的 i18n key 文本。
 */
```
...
> **过渡说明：** 步骤 4.2 中创建的 settings store（`src/stores/settings.ts`）被允许暂时直接调用 `invoke('load_settings')` 和 `invoke('save_settings')`，因为当时 services/svn.ts 尚未创建。本步骤（7.2）完成 services/svn.ts 后，必须将 settings store 中的全部 `invoke` 调用替换为从 `@/services/svn` 导入的 `loadSettings` 和 `saveSettings` 方法。移除 4.2 阶段残留的直接 invoke 调用是迁移到统一入口规范（前端唯一 invoke 入口为 services/svn.ts）的最后一步。
