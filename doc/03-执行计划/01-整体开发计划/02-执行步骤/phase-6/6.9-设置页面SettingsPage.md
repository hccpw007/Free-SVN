...
> **依赖说明：** 本步骤调用 `invoke('get_svn_version')`（对应步骤 3.4 settings.rs 命令注册），需 3.4 步骤已执行完成
...
独立路由 `/settings`（独立布局 `h-screen flex flex-col`，无 TopBar/StatusBar）。顶部导航条[← 返回] + 4 个 El-card 分组 + 关于区（含 SVN 版本号和仓库地址，业务设计 §5.7） + 保存按钮。
...
onMounted(async () => {
  try {
    // 获取 SVN CLI 版本（通过 Rust 后端 get_svn_version 命令或直接 svn --version）
    svnVersion.value = await invoke<string>('get_svn_version')
  } catch {
    svnVersion.value = t('common.unknown')
  }
  // 从 workspaceStore 读取仓库地址（静态导入）
  const ws = useWorkspaceStore()
  repoUrl.value = ws.url || ''
})

// 响应式同步：合并为单个 watch 监听 settingsStore 中所有会同步到 form 的字段（减少独立 watch 调用）
watch(
  () => ({
...
// autoStart 字段（开机自启动）说明：
// 后端需提供 `set_auto_start(enabled: bool)` Tauri command 来完成各平台自启动注册。
// macOS：写入 ~/Library/LaunchAgents/ 下的 plist 文件
// Windows：写入 HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run 注册表
// Linux：创建/删除 ~/.config/autostart/ 下的 .desktop 文件
// 该 command 定义在 auto_start 模块中，并在 lib.rs 的 generate_handler! 中注册。
// 推荐使用 auto_launch crate 跨平台封装。
// 调用方式：
//   import { invoke } from '@tauri-apps/api/core'
...
async function handleSave() {
  ...
  // 同步调用后端 set_auto_start command（开机自启动生效）
  try {
    await invoke('set_auto_start', { enabled: form.autoStart })
  } catch (e) {
    // set_auto_start 失败不应阻止保存流程
    console.warn('Failed to set auto start:', e)
  }
  // langMap: settingsStore.language 值 → vue-i18n locale 映射
  // system → 自动检测浏览器语言; zh-CN → 简体中文; en → English
  // ja → 日本語; ko → 한국어（ja/ko 为 v5 新增语言支持）
  ...
  <div class="h-screen flex flex-col bg-slate-50 dark:bg-slate-900">
    ...
    <div class="h-12 px-4 flex items-center gap-2 border-b border-slate-200
      ...
      <button class="flex items-center gap-1 text-sm text-slate-500
        hover:text-slate-700 dark:hover:text-slate-300 transition-colors focus:ring-2 focus:ring-blue-400 focus:outline-none rounded"
    ...
    <div class="flex-1 overflow-auto p-6 max-w-2xl mx-auto w-full space-y-4">
      ...
      <el-card>
        ...
        <!-- 注：autoStart 的生效需要后端 set_auto_start command 支持（见操作细节中的代码注释）。
             SettingsPage 保存时会同步调用 invoke('set_auto_start', { enabled }) 完成各平台注册。 -->
      </el-card>

      <!-- 关于（业务设计 §5.7：显示 SVN 版本 + 仓库地址；§2.8：安全提示） -->
      <el-card>
        ...
        <div class="space-y-2 text-xs text-slate-400 dark:text-slate-500">
          ...
          <p class="text-amber-500 dark:text-amber-400 mt-2">
            {{ t('settings.securityNotice') }}
          </p>
        </div>
        <el-button size="small" class="mt-3 focus:ring-2 focus:ring-blue-400 focus:outline-none" @click="/* 导出日志 */">{{ t('settings.exportLogs') }}</el-button>
      </el-card>
...
| 占位符 | 说明 | 适用模式 |
|--------|------|---------|
| `<file1>` | 旧版本文件路径（差异对比的左文件） | 差异 |
| `<file2>` | 新版本文件路径（差异对比的右文件） | 差异 |
| `<file>` | 合并后的文件路径 | 合并 |
| `<mine>` | 本地工作副本文件路径 | 合并 |
| `<base>` | 基准版本文件路径（共同祖先） | 合并 |
| `<theirs>` | 服务器版本文件路径 | 合并 |
| `<output>` | 合并结果输出路径 | 合并 |

自定义差异命令示例：`mydiff --left <file1> --right <file2>`
自定义合并命令示例：`mymerge --local <mine> --base <base> --remote <theirs> -o <output>` |
...
- [ ] 关于含版本号 + SVN 版本号（通过 `get_svn_version` 命令调用，依赖步骤 3.4） + 仓库地址 + 三条安全提示 + 导出日志
- [ ] 补充 i18n key：`settings.svnVersion`（SVN 版本号标签）、`settings.repositoryUrl`（仓库地址标签）、`settings.securityTips`（安全提示标题）、`settings.securityTipPlaintext/ClearAdvice/SessionOnly`（三条安全提示）、`settings.securityNotice`（安全注意）
- [ ] 无 `<style>` 标签
