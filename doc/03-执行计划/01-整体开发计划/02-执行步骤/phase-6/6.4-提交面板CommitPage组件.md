...
async function handleCommit() {
  ...
  try {
    ...
    ElMessage.success(t('file.commitSuccess', { rev: result }))
  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex-1 min-h-0 overflow-auto p-4">
      <el-table :data="commitFiles" size="small" style="width:100%" row-key="path" @row-click="(r:any) => toggleFile(r.path)">
        ...
        <el-table-column label="状态" width="48">
...
// 从 svnStore 读取最近日志（上限 20 条），提取 message 作为近期使用日志
onMounted(async () => {
  try {
    const logs = await svnStore.getLog({ limit: 20 })
    recentMessages.value = [...new Set(logs.entries.map((e: any) => e.message).filter(Boolean))]
  } catch { /* 加载最近日志失败不影响提交流程 */ }
})
...
<template>
  <div class="shrink-0 px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
    ...
    <div v-if="recentMessages.length > 0" class="mb-2">
      <el-select v-model="recentMessages[0]" :placeholder="t('file.recentMessages')" size="small" class="!w-full" @change="selectRecent">
...
- [ ] 支持最近使用日志下拉（onMounted 从 svnStore.getLog 加载最近 20 条，点击填充 textarea）
- [ ] 无 `<style>` 标签
