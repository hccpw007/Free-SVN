...
async function handleRetry() {
  ...
  try {
    ...
    if (ok) {
      ...
      statusMessage.value = '认证失败，请检查用户名和密码'
  ...
  >
    <div class="space-y-4">
      ...
      <el-checkbox v-model="saveToCache" :disabled="isProcessing">
        <span class="text-sm">保存凭据到 ~/.subversion/auth/</span>
      </el-checkbox>
...
| manage + 测试连接 | `handleTestConnection()` | 无 | 调用 services/svn.ts 的 `testConnection` |
| manage + 清除缓存 | `handleClearCache()` | 无 | 调用 services/svn.ts 的 `clearCredentials` |
| 点击取消/关闭 | — | `close` | 关闭对话框 |
