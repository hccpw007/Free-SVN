...
pub async fn relocate_repo(
    ...
    // URL scheme 校验（复用上方已定义的 svn_schemes 常量）
    // from_url 和 to_url 的 scheme 必须匹配以下正则之一
    //   /^svn:\/\// — 标准 svn 协议
    //   /^https:\/\// — HTTPS 协议
    //   /^svn\+ssh:\/\// — SVN over SSH 协议
    //   /^http:\/\// — HTTP 协议（用于只读镜像）
    // 不满足任一 scheme 则返回 InvalidInput 错误
...
> **progress 事件说明（参见 3.2 操作规范对照表）：** 长操作（switch/merge/export）当前推送 started/completed/error 三事件。`operation:progress` 逐行推送（含百分比）列为后续增强（TODO(future), 优先级 P3, 触发条件和实现方案见 3.2 要点中的详细说明）。
...
- **merge_branch revision 参数组合（v5 新增参数表）：**

  | revStart | revEnd | 生成的 -r 参数        |
  |----------|--------|-----------------------|
  | 有       | 有     | `-r revStart:revEnd`  |
  | 有       | 无     | `-r revStart:HEAD`    |
  | 无       | 有     | `-r 0:revEnd`         |
  | 无       | 无     | 不使用 -r 参数         |
...
| 测试对象 | 测试场景 | 说明 |
|---------|---------|------|
| switch_branch | 正常切换 / URL 不存在 / 非工作副本 | 验证 targetUrl 校验和 try_lock |
| copy_branch_tag | 正常复制 / 目标已存在 / revision 组合 | 验证 src/dst URL 参数 |
| merge_branch | revStart/revEnd 四种组合 / 合并冲突 | 验证 -r 参数构造逻辑 |
| cleanup_workspace | 正常清理 / 工作副本已锁 | 验证 try_lock/unlock |
| export_workspace | 正常导出 / 路径无效 | 验证 targetDir 参数 |
| lock_files | 正常锁定 / 已锁定 | 验证 paths 和 message 参数 |
| unlock_files | 正常解锁 / 非所有者 | 验证 paths 参数 |
| relocate_repo | 正常搬迁 / URL 格式不合法 | 验证 fromUrl/toUrl 校验 |
| property_ops | get 只读 / set+delete 写锁 / 无 propName | 验证 try_lock 只在写操作时获取 |

**测试方法：** 通过构造参数直接调用 command 函数，验证返回类型和错误处理。args 构造逻辑和 try_lock 条件分支通过单元测试验证。不依赖模拟 Tauri 运行时——纯逻辑测试。
