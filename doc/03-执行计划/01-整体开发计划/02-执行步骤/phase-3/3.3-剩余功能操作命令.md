...
pub async fn cleanup_workspace(
    ...
    state.try_lock()?;
    let r = svn::executor::run_svn(&["cleanup"], &path).await;
    state.unlock(); r
}
...
pub async fn property_ops(
    ...
    match params.action.as_deref().unwrap_or("get") {
        "get" => {
            ...
            svn::executor::run_svn(&["propget", name, &params.path], &params.path).await
        }
        "set" => {
            state.try_lock()?;
            let name = params.prop_name.as_deref().unwrap_or("svn:ignore");
            let value = params.prop_value.as_deref().unwrap_or("");
            let r = svn::executor::run_svn(&["propset", name, value, &params.path], &params.path).await;
            state.unlock(); r
        }
        "delete" => {
            state.try_lock()?;
            let name = params.prop_name.as_deref().unwrap_or("svn:ignore");
            let r = svn::executor::run_svn(&["propdel", name, &params.path], &params.path).await;
            state.unlock(); r
        }
