...
- **长操作进度推送（v5 新增四事件协议）：**
  ...
  - `operation:progress` — 操作执行期间通过异步循环或 svn stdout 逐行解析进度百分比后推送 `{"operation": "checkout", "percent": 45, "stage": "exporting", "file_count": 12}`。当前阶段为简版，svn CLI 输出逐行分析留待后续增强（svn checkout/update 的 stdout 每行包含百分比前缀，可正则提取）
  - `operation:completed` — 操作成功完成时推送 `{"result": "success", "detail": "..."}`
  - `operation:error` — 操作失败时推送 `{"errorCode": "...", "message": "..."}`
  当前 checkout 和 update 的实现已包含 started/completed/error 三个事件，`progress` 事件暂通过操作完成后的单次结果发送替代逐行推送，后续可基于 svn stdout 逐行解析 + `emit("operation:progress")` 增强
