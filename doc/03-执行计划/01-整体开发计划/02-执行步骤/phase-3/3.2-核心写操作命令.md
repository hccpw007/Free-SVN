...
- **长操作进度推送（v5 新增四事件协议）：**
  ...
  当前 checkout 和 update 的实现已包含 started/completed/error 三个事件，`progress` 事件暂不推送。MVP 阶段后端仅通过 started/completed/error 三个事件报告操作状态，`operation:progress` 逐行推送留待后续增强（TODO(future)：优先级 P3，归属迭代 2 增强功能；触发条件：svn CLI 逐行输出中包含百分比前缀如 "  45%" 时触发；实现方案：将 run_svn 改为通过 std::process::Command 的 stdout 管道 + BufReader::read_line() 逐行读取，通过正则 ^\s*(\d+)\s*% 提取百分比，每 200ms 节流后 emit 到前端；前端 ProgressOverlay 组件监听 operation:progress 事件更新进度条百分比和阶段文字）
