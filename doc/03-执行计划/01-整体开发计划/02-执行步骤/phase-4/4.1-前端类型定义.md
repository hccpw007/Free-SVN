...
/** 从后端错误响应提取消息（通过 i18n key 映射获取，回退到中文） */
export function getErrorMessage(err: unknown): string {
  if (err && typeof err === 'object') {
    const e = err as Record<string, unknown>
    if (typeof e.error === 'string') {
      const key = ERROR_MESSAGE_KEYS[e.error as ErrorCode]
      return (key && FALLBACK_MESSAGES_ZH[key]) || (e.message as string) || 'Unknown error'
    }
  }
...
- [ ] error-codes.ts: ErrorCode 枚举 13 个常量 + ERROR_MESSAGE_KEYS（i18n key 映射）+ FALLBACK_MESSAGES_ZH（中文回退）+ getErrorMessage
