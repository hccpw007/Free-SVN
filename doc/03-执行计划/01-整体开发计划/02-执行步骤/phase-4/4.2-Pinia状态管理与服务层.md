...
> **注意：** settings store 直接使用 `invoke` 调用后端 `load_settings` / `save_settings` command（步骤 3.4D 中已注册）。按照规范约定，前端唯一直接调用 `invoke` 的入口应为 `services/svn.ts`（步骤 7.2 中完成全部 26 个方法封装），本 store 在步骤 7.2 完成后应重构为从 `services/svn.ts` 导入方法。当前因执行顺序原因（4.2 先于 7.2 执行），暂允许直接使用 invoke，编码时需在 7.2 完成后添加对应的重构注释。
...
export const useSvnStore = defineStore('svn', () => {
  ...
  async function getLog(path: string, limit?: number, revision?: number, search?: string) { return call<LogEntry[]>('get_log', { path, limit, revision, search }) }
  async function getBlame(path: string, revision?: number) { return call<BlameLine[]>('get_blame', { path, revision }) }
  async function checkout(url: string, targetPath: string, depth?: string, ignoreExternals?: boolean) { return call<string>('checkout_repo', { url, target_path: targetPath, depth, ignore_externals: ignoreExternals }) }
  ...
  async function commit(paths: string[], message: string, keepLocks?: boolean) { return call<OperationResult>('create_commit', { paths, message, keep_locks: keepLocks }) }
  ...
  async function switchBranch(path: string, targetUrl: string, ignoreAncestry?: boolean) { return call<OperationResult>('switch_branch', { path, target_url: targetUrl, ignore_ancestry: ignoreAncestry }) }
  async function copyBranchTag(srcUrl: string, dstUrl: string, message: string, revision?: number) { return call<OperationResult>('copy_branch_tag', { src_url: srcUrl, dst_url: dstUrl, message, revision }) }
  async function mergeBranch(srcUrl: string, revStart?: number, revEnd?: number, targetPath?: string) { return call<OperationResult>('merge_branch', { src_url: srcUrl, rev_start: revStart, rev_end: revEnd, target_path: targetPath }) }
...
- [ ] settings store: 8 字段（含 darkMode）+ load/save + isDirty
- [ ] svn store: 全部 30 个 invoke 方法（含 loadSettings/saveSettings + testConnection/saveCredentials/clearCredentials）+ initEventListeners + 统一错误处理
