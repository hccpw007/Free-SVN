...
export const useSvnStore = defineStore('svn', () => {
  ...
  async function retryAuth(username: string, password: string, saveToCache: boolean): Promise<boolean> {
    ...
    try {
      ...
      const retryArgs = { ...(ctx.args || {}), credentials: { username, password, save_to_cache: saveToCache } }
...
- [ ] svn store: 全部 30 个 invoke 方法 + initEventListeners + 统一错误处理 + SVN_AUTH_FAILED 自动填充 authContext/authFailed → 前端 AuthDialog 联动
- [ ] svn store 导出 retryAuth() / cancelAuth() / authFailed / authContext
