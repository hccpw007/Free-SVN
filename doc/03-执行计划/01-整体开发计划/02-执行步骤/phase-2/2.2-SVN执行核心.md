...
fn run_svn_sync(
    ...
    let child = if let Some(creds) = credentials {
        ...
        // 写入密码到 stdin（在存入 CURRENT_CHILD 之前完成 stdio 接管）
    ...
    // v5 修复：将 child 存入 CURRENT_CHILD 以供 kill_current_process() 取消操作
    // Child 未实现 Clone，通过全局 Mutex 中转所有权
    if let Ok(mut guard) = CURRENT_CHILD.lock() {
        *guard = Some(child);
    }
    // 取出所有权等待完成（取消操作可能已在此处取得并 kill 了进程）
...
6. **`CURRENT_CHILD` 使用说明**：`run_svn_sync()` 在 spawn 子进程后将 child 所有权存入 `CURRENT_CHILD`（通过 `Mutex::lock`），然后立即取出等待完成。取消操作（`kill_current_process()`）可在任意时刻通过 `CURRENT_CHILD.lock()` 取得子进程所有权进行 kill。超时场景下 `timeout` 触发后，子进程所有权仍在 `run_svn_sync` 闭包中，通过 `kill_current_process()` 处理。`run_svn_sync` 执行完毕后 `CURRENT_CHILD` 中为 `None`
