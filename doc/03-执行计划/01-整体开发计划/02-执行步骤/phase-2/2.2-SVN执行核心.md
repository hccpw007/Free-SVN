...
6. **`CURRENT_CHILD` 安全使用规则：**
   - **核心约束**：子进程的 spawn、Child 存取、wait 全部在 `spawn_blocking` 闭包内完成，生命周期不逃逸
   - **简化的存取流程**：`spawn` → `*guard = Some(child)`（Mutex 存入）→ `guard.take()`（取出所有权）→ `child.wait_with_output()`。Guard 持有时间仅限赋值和取值的两个瞬间，不阻塞等待
   - **取消操作**：`kill_current_process()` 通过 `CURRENT_CHILD.lock()` 取得所有权后 kill+wait。若 `run_svn_sync` 恰好已取出 child 正在 wait，则 `CURRENT_CHILD` 中为 `None`，kill 为空操作（进程即将完成）
   - **取消信号**：通过 `AtomicBool` 检测，`run_svn` 入口处检查 `is_cancelled()` 快速返回，闭包内依赖超时机制兜底
   - **关键结论**：Mutex 仅保护赋值/取值的瞬间，不保护 `wait_with_output`，取消操作不被锁阻塞

7. **`AtomicBool + Mutex` 双层取消协作说明：**

   **为什么需要两层：**
   - `AtomicBool`（CANCELLED）提供快速检查的信号层。前端 cancel 按钮触发后立即设置 `CANCELLED=true`，后续新操作的 `run_svn()` 入口检查 `is_cancelled()` 可立即拒绝，无需等待。
   - `Mutex<Option<Child>>`（CURRENT_CHILD）提供进程终止的操作层。实际上终止正在执行的 svn 子进程需要 Child 句柄的所有权，Mutex 保证并发安全。
   
   **双层协作流程：**
   ```
   cancel 触发 → set_cancelled(true)
               → kill_current_process()  // 通过 Mutex 获取 Child 句柄
                                        → child.kill()
                                        → child.wait()  // 等待进程退出
   spawn_blocking 闭包内 → child.wait_with_output()   // 子进程被 kill 后立即返回
   run_svn() 入口         → is_cancelled() 检查        // 新操作被拒绝
   kill_and_cleanup()     → kill + svn cleanup
                          → set_cancelled(false)       // 重置，允许新操作
   ```

   **竞争条件分析（三种情况）：**
   - *情况 A*：cancel 在 `run_svn_sync` spawn 之前到达 → `run_svn()` 入口 `is_cancelled()` 直接返回 Cancelled
   - *情况 B*：cancel 在 child 被存入 CURRENT_CHILD 之后、取出之前到达 → `kill_current_process()` 正常获取 Child 句柄并 kill
   - *情况 C*：cancel 在 child 已被取出（正在 `wait_with_output`）后到达 → `take()` 返回 None，kill 为空操作，进程由其自身完成
   
   情况 B 进程中，虽被 kill，`wait_with_output` 仍能返回（非零退出码），随后被判为 SvnCommand。这属于正常结束，无需特殊处理。
