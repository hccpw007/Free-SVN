...
| `src-tauri/Cargo.toml` | **修改** — 追加 `once_cell = "1.20.2"`、`dirs = "5.0.1"`（若阶段一未添加）|
...
- 依赖：需要 `once_cell = "1.20.2"`、`dirs = "5.0.1"` 两个 crate（确认已在 Cargo.toml 追加）
...
- [ ] `cargo build` 编译通过
- [ ] `config/store.rs` 完整实现：Settings 结构体 11 个字段 + Default + init/load/save/recent/add_recent/current/set_current（新增 diff_command_template/merge_command_template/fallback_to_builtin 与前端 AppSettings 一一对应）
