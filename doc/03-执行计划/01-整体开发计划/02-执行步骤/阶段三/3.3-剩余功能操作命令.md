...
pub async fn property_ops(
    ...
    svn::executor::validate_path(&path)?;
    match action.as_deref().unwrap_or("get") {
        "get" => {
            let name = prop_name.as_deref().unwrap_or("svn:ignore");
            svn::executor::run_svn(&["propget", name, &path], &path).await
        }
        "set" => {
            state.try_lock()?;
            let name = prop_name.as_deref().unwrap_or("svn:ignore");
            let value = prop_value.as_deref().unwrap_or("");
            let r = svn::executor::run_svn(&["propset", name, value, &path], &path).await;
            state.unlock(); r
        }
        "delete" => {
            state.try_lock()?;
            let name = prop_name.as_deref().unwrap_or("svn:ignore");
            let r = svn::executor::run_svn(&["propdel", name, &path], &path).await;
            state.unlock(); r
        }
